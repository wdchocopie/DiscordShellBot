...
def load_config(config_file):
    global TOKEN, GUILD_NAME, CHANNELS_NAME, PASSWORD, SHARED_FOLDER, \
           USERS_FILE, LOG_FILE, LOG_LIMIT, ENABLE_ROOT, UPDATE_COMMAND, \
           UPGRADE_COMMAND, INSTALL_COMMAND, REMOVE_COMMAND
...
def check_config():
    global TOKEN, GUILD_NAME, CHANNELS_NAME, PASSWORD, SHARED_FOLDER, \
           USERS_FILE, LOG_FILE, LOG_LIMIT, ENABLE_ROOT, UPDATE_COMMANDS, \
           UPGRADE_COMMAND, INSTALL_COMMAND, REMOVE_COMMAND
...
async def ask_password(message):
    ...
    await message.author.dm_channel.send("root@linuxrat1234: Enter sudo password.")
...
async def update_system(message):
    ...
    try:
        ...
        proc.wait()
        if proc.poll() == 0:
            output = "root@linuxrat1234: System updated sucessfully."
            await msg_output.edit(content=output)
        else:
            output = "root@linuxrat1234: System not updated, error code: " + str(proc.poll())
            await msg_output.edit(content=output)
    except Exception as e:
        error = "root@linuxrat1234: Error ocurred: " + str(e)
        error_type = "root@linuxrat1234: Error type: " + str((e.__class__.__name__))
...
async def upgrade_system(message):
    ...
    try:
        ...
        if proc.poll() == 0:
            await channel.send("root@linuxrat1234: System upgraded sucessfully.")
        else:
            await channel.send("root@linuxrat1234: System not upgraded" +
...
async def install_package(message):
    ...
    try:
        ...
        if proc.poll() == 0:
            await msg_output.edit(content=f"root@linuxrat1234: Package {message.content} " +
    ...
    try:
        ...
        while True:
            ...
            already_removed = (already_removed or
                               "root@linuxrat1234: 0 to remove" in str(output))
            if output == b'' and proc.poll() is not None:
                break
            await msg_output.edit(content=loading_items[i %
                                  (len(loading_items))] + output_text)
...
async def stop_proccess(message):    # Send ctrl+c to current process
    ...
    if CURRENT_PROCESS:
        ...
        await message.channel.send("root@linuxrat1234: Ctrl + c sent.")
    else:
        await message.channel.send("root@linuxrat1234: There is no process running.")
...
async def on_ready():
    ...
    if guild:
        IN_GUILD = True
        print(f"root@linuxrat1234: Server {GUILD_NAME} found! running...")
        await send_welcome_msg(guild)
        return
    print(f"root@linuxrat1234: Server {GUILD_NAME} not found...")
    exit()
...
async def send_welcome_msg(guild):
    ...
    msg_zero = f"root@linuxrat1234:\n\n ---- LinuxRAT version: {__version__} ----"
    msg_one = "\n\nWelcome to LinuxRAT, this bot allows " + \
...
async def send_command(command, channel):
    ...
    try:
        ...
        error = "root@linuxrat1234: Command not found"
...
async def on_message(message):
    ...
    """
    global USERS_FILELOGIN, VERSION, ENABLE_ROOT, AUTHENTIFIED_USERS, \
        COMMANDS_QUEUE
    ...
    if message.author.id not in AUTHENTIFIED_USERS:
        if message.content == PASSWORD:
            if isinstance(message.channel, discord.channel.DMChannel):
                ...
                response = "root@linuxrat1234: Logged in, you can use commands now."
                await message.author.dm_channel.send(response)
            return
        if not check_user(message.author.id) and in_channel(message):
            await message.author.create_dm()
            response = "root@linuxrat1234: Please log in, insert a valid password."
            await message.author.dm_channel.send(response)
            return
    ...
    if isinstance(message.channel, discord.channel.DMChannel):
        ...
        if allowed:
            ...
            response = "root@linuxrat1234: Wrong password."
    ...
    if message.author.id in COMMANDS_QUEUE['update']:
        ...
        if message.content.strip().lower() == 'yes':
            ...
            response = "root@linuxrat1234: System won't update."
            await message.channel.send(response)
        else:
            response = "root@linuxrat1234: Please reply 'yes' or 'no'."
            await message.channel.send(response)
        return
    ...
    if message.author.id in COMMANDS_QUEUE['upgrade']:
        ...
        if message.content.lower() == 'yes':
            ...
            response = "root@linuxrat1234: System won't upgrade."
            await message.channel.send(response)
        else:
            response = "root@linuxrat1234: Please reply 'yes' or 'no'."
            await message.channel.send(response)
    ...
    if message.author.id in COMMANDS_QUEUE['install']:
        ...
        if message.content.lower() == 'cancel':
            ...
            response = "root@linuxrat1234: No package will be installed."
            await message.channel.send(response)
        else:
    ...
    if message.author.id in COMMANDS_QUEUE['uninstall']:
        ...
        if message.content.lower() == 'cancel':
            ...
            response = "root@linuxrat1234: No package will be removed."
            await message.channel.send(response)
        else:
    ...
    if len(message.attachments) > 0:
        ...
        await message.channel.send(f"root@linuxrat1234: File saved as {file_path}")
        return

    if message.content.lower() == '/update':    # Update system
        await message.channel.send("root@linuxrat1234: Update system? (Write yes/no): ")
        COMMANDS_QUEUE['update'].add(message.author.id)
    elif message.content.lower() == '/upgrade':    # Upgrade system
        await message.channel.send("root@linuxrat1234: Upgrade system? (Write yes/no): ")
        COMMANDS_QUEUE['upgrade'].add(message.author.id)
    elif message.content.lower() == '/install':    # Install package
        await message.channel.send("root@linuxrat1234: Write package name to install or " +
                                   "'cancel' to exit: ")
        COMMANDS_QUEUE['install'][message.author.id] = None
    elif message.content.lower() == '/uninstall':    # Remove package
        await message.channel.send("root@linuxrat1234: Write package name to uninstall or " +
                                   "'cancel' to exit: ")
        ...
        if message.content[0:2] == 'cd':
            try:
                ...
                await message.channel.send("root@linuxrat1234: Changed directory to " +
                                           str(os.getcwd()))
            except Exception as e:
                await message.channel.send(str(e))

        elif "sudo" in message.content and not ENABLE_ROOT:
            await message.channel.send("root@linuxrat1234: Root commands are disabled.")
            ...
            try:
                ...
                if code != 0:
                    text = "root@linuxrat1234: Name or service not known"
            ...
            if os.path.isfile(file_path):
                ...
                await message.channel.send("root@linuxrat1234: File doesn't exists.")
